...
def load_latest(pl_module, save_name, inference=False, map_location=None, silent = False):
    """Loads the last found model from the checkpoints directory

    Args:
        pl_module: PyTorch Lightning module that has the .load_from_checkpoint attribute
    ...
    def find_latest_version(save_name):
        save_loc = os.path.join(CHECKPOINT_PATH, save_name, 'lightning_logs')
    ...
    if os.path.isfile(pretrained_filename):
        ...
        model = pl_module.load_from_checkpoint(pretrained_filename, map_location=map_location)
    else:
